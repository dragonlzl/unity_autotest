...
#         print(self._resultForDoCleanups.failures)
#         if self._resultForDoCleanups.failures or self._resultForDoCleanups.errors:
#             raise SkipTest(
#                 "{} do not excute because {} is failed".format(func.__name__,
#                  self._resultForDoCleanups.failures[ 0][0]._testMethodName))
#         func(self)
#
#     return RebackTest
    ...
    def RebackTest(self):
        ...
        try:
            ...
            if not isExists:
                os.makedirs(dir)
                print(dir)
            filename = PATH(os.path.join(dir, str(os.getpid())[0:3] + str(int(time.time())) + '.png'))
            snapshot(filename)
            name = filename.split('\\')
            print('bug_image:', os.path.join(name[-2], name[-1]))
